...
# Read the data.
file_name <- 'active_back_up_3600'
cat(paste('Cleaning: \'', build_product_input_file_path(file_name), '\'...',
          sep = ''))
active_back_up_3600 <- read_product_csv(file_name, my_quote = target_char)
...
# Set column names in the active_back_up_3600 data.
colnames(active_back_up_3600) <- my_colnames

# Create the POSIX fields in the data.
active_back_up_3600[,date_fields_posix] <- sapply(seq(1:length(date_fields)),
                                                  FUN = function(x) {

  create_timestamp(active_back_up_3600[,date_fields[x]])
})

# Create the normalized currency fields in the data.
active_back_up_3600[,currency_fields_norm] <- sapply(seq(1:length(currency_fields)),
                                                    FUN = function(x) {

  calculate_normalized_price(active_back_up_3600[,currency_fields[x]],
                             active_back_up_3600[,date_fields_posix[1]])
})

# Round the other currency fields.
active_back_up_3600[,currency_fields] <- sapply(seq(1:length(currency_fields)),
                                                FUN = function(x) {

  round_currency(active_back_up_3600[,currency_fields[x]])
})

# Substitute quote characters in the description field.
active_back_up_3600[,target_column] <- gsub(get_quote_char(),
                                            target_char,
                                            active_back_up_3600[,target_column])

# Reorder the active_back_up_3600s data and write the csv file.
write_product_csv(active_back_up_3600[,my_enhanced_colnames], file_name)
