...
# Read the data.
file_name <- 'pp_returned_products'
cat(paste('Cleaning: \'', build_sales_input_file_path(file_name), '\'...',
          sep = ''))
pp_returned_products <- read_sales_csv(file_name, my_quote = target_char)
...
# Set column names in the pp_returned_products data. Substitute quote characters
# in the description field.
colnames(pp_returned_products) <- my_colnames
pp_returned_products[,target_column] <- gsub(get_quote_char(),
                                        target_char,
                                        pp_returned_products[,target_column])

# Create the POSIX fields in the data.
pp_returned_products[,date_fields_posix] <- sapply(seq(1:length(date_fields)),
                                                   FUN = function(x) {
                                                
  create_timestamp(pp_returned_products[,date_fields[x]])
})

# Create the normalized date-in currency fields in the data.
pp_returned_products[,date_in_currency_fields_norm] <- sapply(
  seq(1:length(date_in_currency_fields)), FUN = function(x) {

  calculate_normalized_price(pp_returned_products[,date_in_currency_fields[x]],
                             pp_returned_products[,date_fields_posix[1]])
})

# Create the normalized date-sold currency fields in the data.
pp_returned_products[,date_sold_currency_fields_norm] <- sapply(
  seq(1:length(date_sold_currency_fields)), FUN = function(x) {
    
    calculate_normalized_price(
      pp_returned_products[,date_sold_currency_fields[x]],
      pp_returned_products[,date_fields_posix[2]])
  })

# Round the other date-in currency fields.
pp_returned_products[,date_in_currency_fields] =
  sapply(seq(1:length(date_in_currency_fields)),
         FUN = function(x) {
           
  round_currency(pp_returned_products[,date_in_currency_fields[x]])
})

# Round the other date-sold currency fields.
pp_returned_products[,date_sold_currency_fields] =
  sapply(seq(1:length(date_sold_currency_fields)),
         FUN = function(x) {
           
           round_currency(pp_returned_products[,date_sold_currency_fields[x]])
         })

# Reorder the pp_returned_products data and write the csv file.
write_sales_csv(pp_returned_products[,my_enhanced_colnames], file_name)
